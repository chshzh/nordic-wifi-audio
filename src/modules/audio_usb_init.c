...
/* Full-Speed configuration (all boards). */
USBD_CONFIGURATION_DEFINE(audio_fs_config, attributes, CONFIG_USBD_MAX_POWER, &fs_cfg_desc);
/* High-Speed configuration (registered only on HS-capable controllers, e.g. nRF54LM20DK). */
USBD_CONFIGURATION_DEFINE(audio_hs_config, attributes, CONFIG_USBD_MAX_POWER, &hs_cfg_desc);
...
static struct usbd_context *audio_usbd_setup_device(usbd_msg_cb_t msg_cb)
{
	...
	/* Register the High-Speed configuration first on HS-capable controllers
	 * (nRF54LM20DK usbhs). Full-Speed-only controllers (nRF5340 usbd) skip this.
	 */
	if (USBD_SUPPORTS_HIGH_SPEED && usbd_caps_speed(&audio_usbd) == USBD_SPEED_HS) {
		ret = usbd_add_configuration(&audio_usbd, USBD_SPEED_HS, &audio_hs_config);
		if (ret) {
			LOG_ERR("Failed to add High-Speed configuration");
			return NULL;
		}

		ret = usbd_register_all_classes(&audio_usbd, USBD_SPEED_HS, 1, blocklist);
		if (ret) {
			LOG_ERR("Failed to register High-Speed classes");
			return NULL;
		}

		usbd_device_set_code_triple(&audio_usbd, USBD_SPEED_HS, USB_BCC_MISCELLANEOUS, 0x02,
					    0x01);
	}
	...
	if (ret) {
		LOG_ERR("Failed to register Full-Speed classes");
		return NULL;
	}
